  ...
  if (sorted.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-sm font-bold uppercase text-[var(--c-black)]">Событий не найдено</p>
        <p className="text-xs mt-1 text-[#999]">Попробуйте изменить фильтры</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {sorted.map((event, idx) => (
        <EventCard key={event.id} event={event} onClick={onEventClick} colorIndex={idx} />
      ))}
    </div>
  );
}
