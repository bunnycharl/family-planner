...
export function TaskDetailSheet({
  ...
  const startFormatted = format(new Date(task.startDate), "d MMM yyyy", { locale: ru });
  const endFormatted = format(new Date(task.endDate), "d MMM yyyy", { locale: ru });
    ...
    <div className="fixed inset-0 z-[60]">
      ...
      >
          ...
          <div className="flex-1">
            ...
            {typeInfo && (
              <div className="mt-1.5 flex items-center gap-2">
                ...
                />
                <span className="text-xs font-bold text-[#999]">{typeInfo.label}</span>
              </div>
            )}
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-3 space-y-3">
          ...
          {/* Details */}
          {task.details && <p className="text-sm text-[#666] leading-relaxed">{task.details}</p>}
