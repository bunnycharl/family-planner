...
export function TaskFormModal({
  ...
  useEffect(() => {
    if (isOpen) {
      if (task) {
        ...
        setName("");
        setDetails("");
        setTaskType(taskTypes.length > 0 ? taskTypes[0].key : "");
        setStartDate("");
        setEndDate("");
  ...
  }, [isOpen, task, defaultPhaseId, phases, taskTypes]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const url = isEditing ? `/api/roadmap/tasks/${task!.id}` : "/api/roadmap/tasks";
